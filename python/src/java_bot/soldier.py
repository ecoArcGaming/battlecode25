...
class Soldier(Robot):
    ...
    def update_enemy_tiles(nearby_tiles):
        ...
        closest_enemy_tower = Sensing.tower_in_range(20, False)
        ...
        enemy_paint = Sensing.find_enemy_paint(nearby_tiles)
    ...
    def update_state(cur_location, nearby_tiles):
        ...
        if (Soldier.has_low_paint(constants.LOW_PAINT_THRESHOLD) and 
            ...
            globals()['enemy_tile'] = Soldier.update_enemy_tiles(nearby_tiles)
            ...
            # Check for nearby buildable ruins if we are not currently building one
                ...
                best_ruin = Sensing.find_best_ruin(cur_location, nearby_tiles)
    ...
    def update_state_osama(cur_location, nearby_tiles):
        ...
        if Soldier.has_low_paint(constants.LOW_PAINT_THRESHOLD):
            ...
            globals()['enemy_tile'] = Soldier.update_enemy_towers(nearby_tiles)
            if globals()['enemy_tile'] is not None and globals()['last_tower'] is not None:
                globals()['soldier_type'] = SoldierType.ADVANCE
                Soldier.reset_variables()
            if globals()['soldier_state'] != SoldierState.FILLINGTOWER:
                best_ruin = Sensing.find_any_ruin(cur_location, nearby_tiles)
                if best_ruin is not None:
                    if not Sensing.can_build_tower(best_ruin.get_map_location()):
                        globals()['soldier_type'] = SoldierType.ADVANCE
                        Soldier.reset_variables()
                    else:
                        globals()['ruin_to_fill'] = best_ruin.get_map_location()
            ...
            # Turn into an advance bot if they see an enemy paint that prevents tower building
                ...
                if not Sensing.can_build_tower(globals()['ruin_to_fill']):
                    globals()['soldier_type'] = SoldierType.ADVANCE
                    Soldier.reset_variables()
    ...
    def update_srp_state(cur_location, nearby_tiles):
        ...
        if (globals()['soldier_state'] != SoldierState.LOWONPAINT and 
            ...
            if (get_map_width() <= constants.SRP_MAP_WIDTH and 
                ...
                if can_build_srp and len(poss_srp) == 25 and not Sensing.conflicts_srp():
                    Soldier.reset_variables()
                    globals()['soldier_state'] = SoldierState.FILLINGSRP
    ...
    def msg_tower():
        ...
        if can_sense_robot_at_location(tower_location) and can_send_message(tower_location):
            Communication.send_map_information(globals()['enemy_tile'], tower_location)
    ...
    def fill_in_ruin(ruin_location):
        ...
        if not Sensing.can_build_tower(ruin_location):
        ...
        # Check to see if we know the type of tower to fill in
            ...
            Soldier.complete_ruin_if_possible(ruin_location)
        else:
    ...
    def stuck_behavior():
        ...
        if globals()['soldier_type'] in [SoldierType.DEVELOP, SoldierType.SRP]:
            new_dir = Pathfinding.find_own_corner()
        else:
            new_dir = Pathfinding.get_unstuck()
