...
class Sensing:
    ...
    def find_best_ruin(robot_location, nearby_tiles):
        ...
        for tile in nearby_tiles:
            if tile.has_ruin():
                tile_location = tile.get_map_location()
                if (not can_sense_robot_at_location(tile_location) and 
                    Sensing.can_build_tower(tile_location) and
                    len(sense_nearby_robots(tile_location, 2, get_team())) < 1):
                    ruin_distance = robot_location.distance_squared_to(tile_location)
                    if min_dis == -1 or min_dis > ruin_distance:
                        cur_ruin = tile
    ...
    def score_splasher_tiles():
        ...
        best_score = -1
        for tile in nearby_tiles:
            if tile.is_passable() and can_attack(tile.get_map_location()):
                score = Sensing.score_splash(tile)
                if score > best_score:
                    best_score = score
                    best = tile
        ...
        if best is None:
            ...
            for dx, dy in [(0, 4), (4, 0), (0, -4), (-4, 0)]:
                if 0 <= x + dx < get_map_width() and 0 <= y + dy < get_map_height():
                    tile = sense_map_info(get_location().translate(dx, dy))
                    score = Sensing.score_splash(tile)
                    if score > best_score:
                        best_score = score
                        best = tile
